class Test# < ActiveRecord::Base
  ...
  #******** initialize is not valid method call in Rails? 
  ...
  def check_horizontal(board, start_column, row)
    match_counter = 0
    column = start_column
    unless column - 1 < 0 || board[column][row] != board[column - 1][row] || match_counter == 3
      match_counter += 1
      column -= 1
    end
    column = start_column
    unless column + 1 > 6 || board[column][row] != board[column + 1][row] || match_counter == 3
      match_counter += 1
      column += 1
    end
    if match_counter == 3
